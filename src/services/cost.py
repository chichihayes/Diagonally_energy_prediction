...
def project_weekly_bill(forecast: list[dict]) -> dict:
    tariff = float(os.environ["ELECTRICITY_TARIFF_GBP_PER_KWH"])
    optimistic_gbp = round(
        sum(r["lower_wh"] for r in forecast) / 1000 * tariff, 2
    )
    most_likely_gbp = round(
        sum(r["predicted_wh"] for r in forecast) / 1000 * tariff, 2
    )
    pessimistic_gbp = round(
        sum(r["upper_wh"] for r in forecast) / 1000 * tariff, 2
    )
    return {
